...
class BaseModel(nn.Module):
    ...
    """
    def __init__(self, one_hot_feature_dims, embed_dim, multi_hot_dict, l2_reg_embedding=1e-5, l2_reg_linear=1e-5):
        super(BaseModel, self).__init__()
        self.multi_hot_flag = multi_hot_dict['multi_hot_flag']
        self.feature_dims = one_hot_feature_dims + sum(self.multi_hot_flag)
        self.embedding = FeaturesEmbedding(one_hot_feature_dims, embed_dim, multi_hot_dict)
        self.embed_output_dim = self.embedding.output_dim0 * embed_dim
        self.embed_dim = embed_dim
        self.field_num = self.embedding.one_hot_field_num + self.embedding.multi_hot_field_num

        self.linear = FeaturesLinear(self.embed_output_dim)  # linear+bias
...
class FeaturesEmbedding(torch.nn.Module):
    ...
    def __init__(self, one_hot_field_dims, embed_dim, multi_hot_dict=None):
        super().__init__()

        self.multi_hot_flag = np.array(multi_hot_dict['multi_hot_flag'])
        self.one_hot_field_num = len(one_hot_field_dims)
        self.multi_hot_field_num = sum(self.multi_hot_flag)//multi_hot_dict['seq_maxlen']
        self.seq_maxlen = multi_hot_dict['seq_maxlen']
        self.multi_hot_method = multi_hot_dict['method']
        if self.multi_hot_method not in {'sum', 'mean', None}:
            raise ValueError(f"Invalid multi-hot method '{self.multi_hot_method}'. Method must be 'mean', 'sum', or None.")
        self.output_dim0 = self.one_hot_field_num + (self.multi_hot_field_num
                                                     if self.multi_hot_method in {'sum', 'mean'} else
                                                     sum(self.multi_hot_flag))
        self.embed_dim = embed_dim
        print(f'one_hot_field_num: {self.one_hot_field_num}, ', end='')
        print(f'multi_hot_field_num: {self.multi_hot_field_num}, ', end='')
        # print(f'one_hot_field_dims: {one_hot_field_dims.values}, ', end='')
        print(f'embed_dim: {self.embed_dim}')

        self.embedding_dict = torch.nn.Embedding(sum(one_hot_field_dims), embed_dim)
        if np.__version__ >= '1.20.0':
            self.offsets = np.array((0, *np.cumsum(one_hot_field_dims)[:-1]), dtype=np.longlong)
        else:
            self.offsets = np.array((0, *np.cumsum(one_hot_field_dims)[:-1]), dtype=np.long)
        if self.multi_hot_field_num > 0:
            multi_hot_offsets = [self.offsets[multi_hot_dict['itemid_idx']]] * sum(self.multi_hot_flag)
            self.offsets = np.concatenate((self.offsets, multi_hot_offsets))
    ...
    def forward(self, x, squeeze_dim=False):
        ...
        if (self.multi_hot_field_num > 0) and (self.multi_hot_method in {'mean', 'sum'}):
            one_hot_embed_x = embed_x[:, ~self.multi_hot_flag, :]
            multi_hot_embed_x = embed_x[:, self.multi_hot_flag, :]
            reshaped_multi_hot_embed_x = multi_hot_embed_x.view(-1, self.multi_hot_field_num, self.seq_maxlen, self.embed_dim)

            if self.multi_hot_method == 'mean':
                pooled_multi_hot_embed_x = torch.mean(reshaped_multi_hot_embed_x, dim=2)
            else:
                pooled_multi_hot_embed_x = torch.sum(reshaped_multi_hot_embed_x, dim=2)

            embed_x = torch.cat((one_hot_embed_x, pooled_multi_hot_embed_x), dim=1)
