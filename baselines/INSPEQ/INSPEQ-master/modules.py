...
class GRUCell(nn.Module):
    ...
    def forward(self, input, hidden):
        gates = F.linear(input, self.weight_ih, self.bias_ih) + F.linear(hidden, self.weight_hh, self.bias_hh)
...
class GRULayer(nn.Module):
    ...
    def forward(self, input, hidden=None):
        ...
        if hidden is None:
            hidden = input.new_zeros(self.num_layers * (2 if self.bidirectional else 1), batch_size, self.hidden_size)
        # 初始化隐藏状态
        ...
        for i in range(self.num_layers):
            ...
            for direction in range(2 if self.bidirectional else 1):
                layer_hidden = hidden[i, direction]
                layer_output = []
                if direction == 1:  # 反向传播
                    output = output.flip(1)
                for t in range(seq_len):
                    layer_hidden = self.gru_cells[i * (2 if self.bidirectional else 1) + direction](output[:, t, :], layer_hidden)
                    layer_output.append(layer_hidden)
                if direction == 1:  # 反向传播
                    layer_output = layer_output[::-1]
                layer_output = torch.stack(layer_output, dim=1)
                all_hidden.append(layer_output)
                final_hidden.append(layer_hidden)
            output = torch.cat(all_hidden, dim=2)
...
class paraGRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, bidirectional=False, dropout=0.0):
        super(paraGRU, self).__init__()
        self.gru_layer = GRULayer(input_size, hidden_size, num_layers, bidirectional, dropout)

    def forward(self, input, hidden=None):
        return self.gru_layer(input, hidden)
